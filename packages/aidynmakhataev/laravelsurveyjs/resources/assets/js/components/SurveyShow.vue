...
export default {
    ...
    mounted() {
        this.survey.onComplete.add(result => {
            ...
            axios
                .post(url, { json: result.data })
                .then(response => {
                    console.log(response);
                    this.survey.clear();
                    this.$toastr.s("Your Answers were Successfully Saved");
                    window.setTimeout(  this.$toastr.i("Please fill in other answers"), 3000);
                    this.survey.render();
                })
                .catch(errors => {
                    this.survey.clear();
                    this.$toastr.e("An error Occured while saving");
                    this.survey.render();
                });
        });
    }
